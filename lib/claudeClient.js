...
async function callClaudeForOutfits(systemPrompt, userPrompt) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    if (parsed) {
      ...
      console.error('Outfit JSON failed schema validation:', errors.join('; '));
      if (attempt === 1) {
        messages = [
      ...
      console.error('Outfit response was not valid JSON. Raw text (first 500 chars):', rawText.slice(0, 500));
      messages = [
        { role: 'user', content: userPrompt },
        {
          role: 'user',
      ...
      console.error('Outfit response STILL not valid JSON after retry. Raw text (first 800 chars):', rawText.slice(0, 800));
    }
  }
